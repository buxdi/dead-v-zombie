				...
				ESX.UI.Menu.Open(
					...
					function(data2, menu2)
						...
						if count == nil or count <= 0 then
							...
							TriggerServerEvent('sp_fueler:getStockItem', itemName, count)
						end
...
function OpenPutStocksMenu()
	...
	ESX.TriggerServerCallback('sp_fueler:getPlayerInventory', function(inventory)
				...
				ESX.UI.Menu.Open(
					...
					function(data2, menu2)
						...
						if count == nil or count <= 0 then
							...
							TriggerServerEvent('sp_fueler:putStockItems', itemName, count)
						end
...
AddEventHandler('sp_fueler:hasEnteredMarker', function(zone)
...
AddEventHandler('sp_fueler:hasExitedMarker', function(zone)
	ESX.UI.Menu.CloseAll()
	if (zone == 'PetrolFarm') and PlayerData.job ~= nil and PlayerData.job.name == 'fueler' then
		TriggerServerEvent('sp_fueler:stopHarvest')
	end  
	if (zone == 'TraitementRaffin' or zone == 'TraitementPetrol') and PlayerData.job ~= nil and PlayerData.job.name == 'fueler' then
		TriggerServerEvent('sp_fueler:stopTransform')
	end
	if (zone == 'SellFarm') and PlayerData.job ~= nil and PlayerData.job.name == 'fueler' then
		TriggerServerEvent('sp_fueler:stopSell')
	end
	CurrentAction = nil
end)
	...
	while true do
		...
		if PlayerData.job ~= nil and PlayerData.job.name == 'fueler' then
			...
			if (isInMarker and not HasAlreadyEnteredMarker) or (isInMarker and LastZone ~= currentZone) then
				...
				TriggerEvent('sp_fueler:hasEnteredMarker', currentZone)
			end

			if not isInMarker and HasAlreadyEnteredMarker then
				HasAlreadyEnteredMarker = false
				TriggerEvent('sp_fueler:hasExitedMarker', LastZone)
			end
		end
	end
end)
		...
		if CurrentAction ~= nil then
			...
			if IsControlPressed(0,  Keys['E']) and PlayerData.job ~= nil and PlayerData.job.name == 'fueler' and (GetGameTimer() - GUI.Time) > 300 then
				if CurrentAction == 'raisin_harvest' then
					TriggerServerEvent('sp_fueler:startHarvest', CurrentActionData.zone)
				end
				if CurrentAction == 'jus_traitement' then
					TriggerServerEvent('sp_fueler:startTransform', CurrentActionData.zone)
				end
				if CurrentAction == 'vine_traitement' then
					TriggerServerEvent('sp_fueler:startTransform', CurrentActionData.zone)
				end
				if CurrentAction == 'farm_resell' then
					TriggerServerEvent('sp_fueler:startSell', CurrentActionData.zone)
				end
