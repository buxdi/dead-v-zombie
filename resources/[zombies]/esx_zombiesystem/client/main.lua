		...
		for i, entity in pairs(entitys) do
			for j, player in pairs(players) do
				...
				if distance <= 17.0 then
					...
					TaskGoToEntity(entity, GetPlayerPed(player), -1, 1.0, 4.0, 1073741824, 0)
				end
			end
		end
	end
			...
			if IsPedDeadOrDying(entity, 1) == 1 then
				...
				if IsPedRagdoll(entity, 1) ~= 1 then
					if not IsPedGettingUp(entity) then
						if(Vdist(playerX, playerY, playerZ, pedX, pedY, pedZ) < 1)then
							RequestAnimDict("misscarsteal4@actor")
							TaskPlayAnim(entity,"misscarsteal4@actor","stumble",2.5, 0.7, 5, 9, 1.0, 0, 0, 0)
							if(Vdist(playerX, playerY, playerZ, pedX, pedY, pedZ) < 0.7)then
								local playerPed = (GetPlayerPed(-1))
								local maxHealth = GetEntityMaxHealth(playerPed)
								local health = GetEntityHealth(playerPed)
								local newHealth = health - 35
								TriggerEvent('esx_status:add', 'stress', 100000)
								TriggerEvent('esx_status:remove','immunesystem', 7000)
								SetEntityHealth(playerPed, newHealth)
								--TaskGoStraightToCoord(entity, playerX, playerY, playerZ, 1.0, 0, 0,0)
							end
							Wait(1000)
							TaskGoToEntity(entity, GetPlayerPed(-1), -1, 0.3, 4.0, 1073741824, 0)
						end
					end
				end
			end
					...
					if GetPedSourceOfDeath(entity) == PlayerPedId() then
						...
						if not IsPedInAnyVehicle(PlayerPedId(), false) then
							if(Vdist(playerX, playerY, playerZ, pedX, pedY, pedZ) < 1.5) then
								ESX.Game.Utils.DrawText3D({x = pedX, y = pedY, z = pedZ + 0.2}, '~y~?', 0.8, 4)
								if IsControlJustReleased(1, 51) then
									if DoesEntityExist(GetPlayerPed(-1)) then
										...
										if randomChance > 0 and randomChance < Config.ProbabilityWeaponLoot then
											TriggerServerEvent('esx_zombiesystem:weaponloot', randomWeapon)
										...
										elseif randomChance >= Config.ProbabilityItemLoot and randomChance < 100 then
											--ESX.ShowNotification('~y~Rien...')
											ESX.ShowNotification("Rien", "error", 3000)
										end
