...
Citizen.CreateThread(function()
    while true do
        ...
        for k, v in pairs(Config['FleecaHeist']) do
            local object2 = GetClosestObjectOfType(Config['FleecaHeist'][k]['scenePos'], 50.0, GetHashKey('hei_prop_heist_sec_door'), 1, 0, 0)
            sleep = 1
            if HasEntityBeenDamagedByWeapon(object2, 0x2C3731D9, 0) then
                StartFleecaHeist(k)
                Wait(5000)
            end
        end
