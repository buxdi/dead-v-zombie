...
RegisterNetEvent('px_crafting2:showCrafting')
AddEventHandler('px_crafting2:showCrafting', function()
    ShowAllEntity()
end)

function ShowAllEntity()
    local data = lib.callback.await('px_crafting2:getTablePosition', false)
...
function InfoEntity(coords, name)
    lib.registerMenu({
        ...
        elseif selected == 2 then
            view = false
            TriggerServerEvent('px_crafting2:DeleteEntity', coords, name)
...
function OpenMenuCraft(hash, value)
    ...
    lib.registerMenu({
        ...
        if selected == totalOptions then
            local data = lib.callback.await('px_crafting2:getItemCount', false, args.craftingInfo)
                ...
                if Crafting.XpSystem then
                    if data.xp >= xp then
                        ...
                        if lib.progressBar({
                            ...
                            CamOFF()
                            TriggerServerEvent('px_crafting2:removeItem', item, itemName)
                        end
                    else
...
RegisterNetEvent('px_crafting2:placeProp')
AddEventHandler('px_crafting2:placeProp', function(prop)
    debug('Hola')
    prop = joaat(prop)
    heading = 0.0
    confirmed = false
        ...
        while not confirmed do
            ...
            if IsControlJustPressed(0, 38) then -- "E"
                ...
                if not input then
                    ...
                    TriggerServerEvent('px_crafting2:SaveTable', input[1], coords.x, coords.y, coords.z, heading)
