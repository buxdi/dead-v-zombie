...
if PET.ManualMode then
    RegisterCommand("pets", function()
        ESX.TriggerServerCallback('lab-pet:server:myPets', function (mypets)
            if mypets then
                SendNUIMessage({
                    action = "MY_PETS",
                    petlist = mypets
                ...
                TriggerEvent('lab-pet:client:sendNotify', Locales.NoAnimal)
            end
        end)
    end)
end
                            ...
                            if dist <= 1.3 then
                                if dist <= 1.2 then
                                    ...
                                    if PET.debugMode then
                                        print('::NEARBY PED NETWORK ID:: =>', NetworkGetNetworkIdFromEntity(v))
                                    end  
                                    if pets[NetworkGetNetworkIdFromEntity(v)] ~= nil then
                                        if pets[NetworkGetNetworkIdFromEntity(v)].netID  == NetworkGetNetworkIdFromEntity(v) then
                                            ...
                                            SendNUIMessage({
                                                ...
                                                petLevel = pets[NetworkGetNetworkIdFromEntity(v)].petLevel,
                                            ...
                                            if IsControlJustPressed(0,PET.petInteractKeyCode) then
                                                if pets[NetworkGetNetworkIdFromEntity(v)].petOwner == ESX.GetPlayerData().identifier then
                                                    SetNuiFocus(true, true)
...
boughtScreen = function()
    ESX.TriggerServerCallback('lab-pet:server:getPlayersSellingPets', function (data, citizenid)
...
RegisterCommand(PET.showPetCommand, function()
    for k,v in pairs(pets) do
        if v.petOwner == ESX.GetPlayerData().identifier then
...
RegisterNUICallback('changePet', function(data,cb)
    ...
    if data.secondHand then
        dog = CreatePed(9, pedHash, PET.SecondHandMarketShowCoords.x, PET.SecondHandMarketShowCoords.y, PET.SecondHandMarketShowCoords.z-1.0, PET.SecondHandMarketShowCoords.h, false, true)
    else
...
RegisterNUICallback('RemoveFromList', function(data,cb)
    TriggerServerEvent('lab-pet:server:RemmoveFromList', data)
    SendNUIMessage({
        action = "CLOSE_KEY"
    })
end)

RegisterNUICallback('getPet', function(data,cb)
    if dog ~= nil then
        DeleteEntity(dog)
    end
    TriggerServerEvent('lab-pet:server:getPet', data)
end)

RegisterNetEvent('lab-pet:client:backPet', function(animalType, animalNetworkID)
    TriggerServerEvent('lab-pet:server:backPet', tonumber(animalNetworkID))
    SendNUIMessage({
        action = "CLOSE_KEY"
    })
    Wait(2000)
...
RegisterNetEvent('lab-pet:client:sellPet', function(animalType, animalNetworkID)
    SendNUIMessage({
        action = "SELL_PET",
        animalType = animalType,
        animalNetworkID = animalNetworkID
    })
    SetNuiFocus(true, true)
end)

RegisterNUICallback("sellPet",function(data)
    TriggerServerEvent('lab-pet:server:backPet', data)
    SendNUIMessage({
        action = "CLOSE_KEY"
    })
    Wait(2000)
