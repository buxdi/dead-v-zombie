...
$('.pet-buybox').click(function (e) { 
    ...
    if ($('.pet-buybox').text() == "Cancel") {
        $.post("https://lab-pet/RemoveFromList", JSON.stringify({petOwner, mynetID}));     
    } else {
        $.post("https://lab-pet/buyPet", JSON.stringify({petOwner, petPrice, petName,petLabel,petHungryLevel,petThirstLevel,petEnergyLevel,petHealthLevel,petGender,petLevel,pedHash,hungryDecrase,thirstDecrase,animalList,listOf, petXP, lastXP,petTexureID}));     
    } 
});
...
$('.mypet-buybox').click(function (e) { 
    ...
    listOf = animalList
    if (isOutSide == 0) {
        $.post("https://lab-pet/getPet", JSON.stringify({mynetID, petName,petLabel,petHungryLevel,petThirstLevel,petEnergyLevel,petHealthLevel,petGender,petLevel,pedHash,hungryDecrase,thirstDecrase,animalList,listOf, petXP, lastXP,petTexureID}));     
    }
});
    ...
    if ($('.pet-input-area-header').text() == "How much do you want to sell?") 
    {
        if (Number(petNewIMG) > 0) {
            if (myanimalNetworkID && myanimalType) {
                $.post("https://lab-pet/sellPet", JSON.stringify({myanimalType, myanimalNetworkID, petNewIMG}));
